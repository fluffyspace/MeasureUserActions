...
abstract class ExpandableBubbleService : FloatingBubbleService() {
    ...
    var fingerDownX: Float = 0f
    var fingerDownY: Float = 0f
    var distanceToTriggerCloseButton: Int = 0
    var fingerUp: Boolean = true
    ...
    private inner class CustomBubbleListener(
        ...
        override fun onFingerMove(x: Float, y: Float) {
            ...
            if (fingerUp.not() && isCloseBubbleEnabled && isCloseBubbleVisible.not() && ((x - fingerDownX) > distanceToTriggerCloseButton || (y - fingerDownY) > distanceToTriggerCloseButton)) {
                isCloseBubbleVisible = true
                Log.d("ingo", "show bubble")
